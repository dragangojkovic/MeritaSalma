    ...
    <a class="nav-link"  href="../customer/MyPurchases.aspx?status=Cancel"" >
     ...
     <asp:Label ID="Label6" runat="server" Text='<%# Eval("ProfileId") %>' Visible="false" /> 
                ...
                <div class="inbox row">
                     ...
                     <div class="col-md-1">
                            <asp:Label ID="Label7" runat="server" Text="Qty: " Font-Bold="True" ForeColor="Gray" Font-Size="X-Small" CssClass="hidden-lg"></asp:Label>
                           <asp:Label ID="Label8" runat="server" Text='<%# Eval(" deliverycostperitem" ) %>' />
                            <div class="hidden-lg" style="padding: 3px; border-bottom: 1px solid #ccc; margin-bottom: 10px;"></div>
                        </div>
...
<asp:Button ID="Button2" runat="server" Text="Contact" CommandName="contact" CssClass="btn btn-success btn-xs" />
    ...
    <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:DefaultConnection %>" ProviderName="<%$ ConnectionStrings:DefaultConnection.ProviderName %>" SelectCommand="SELECT AspNetUsers_1.UserName AS BuyerUserName, dbo.[Order].OrderNo, dbo.[Order].OrderDate, dbo.[Order].Amount, dbo.[Order].Requirement, dbo.AspNetUsers.UserName, dbo.Products.ProductName, dbo.Products.CoverImage, dbo.[Order].Quantity, dbo.[Order].Quantity * dbo.[Order].Amount + dbo.[Order].Quantity * dbo.[Order].deliverycostperitem AS TotalAmount, ISNULL(dbo.Delivery.ActionStatus, dbo.[Order].Status) AS Status, dbo.Profile.ProfileId, dbo.Currency.Currency, dbo.[Order].deliverycostperitem FROM dbo.[Order] INNER JOIN dbo.AspNetUsers ON dbo.[Order].SellerId = dbo.AspNetUsers.Id INNER JOIN dbo.AspNetUsers AS AspNetUsers_1 ON dbo.[Order].BuyerId = AspNetUsers_1.Id INNER JOIN dbo.Profile ON dbo.AspNetUsers.UserName = dbo.Profile.UserName INNER JOIN dbo.Products ON dbo.[Order].ProductKey = dbo.Products.ProductKey INNER JOIN dbo.Currency ON dbo.Products.Currency = dbo.Currency.CurrencyId LEFT OUTER JOIN dbo.Delivery ON dbo.[Order].OrderNo = dbo.Delivery.OrderNo WHERE (AspNetUsers_1.UserName = @UserName) AND (ISNULL(dbo.Delivery.ActionStatus, dbo.[Order].Status) LIKE N'%' + @status + N'%')">
        <SelectParameters>
