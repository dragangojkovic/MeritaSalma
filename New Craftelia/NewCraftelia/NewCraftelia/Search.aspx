 ...
 <div>
    ...
    <ItemTemplate> 
      <asp:Label ID="Label2" runat="server" Text='<%# Eval("CategoryId") %>' Visible="False"></asp:Label>  
        <li class="dropdown">
          <a href="#" class="dropdown-toggle" data-toggle="dropdown">  <asp:Image ID="Image8" runat="server" ImageUrl='<%# Eval("Icon") %>' Height="15px" ImageAlign="AbsMiddle" Visible="false" /> 
  ...
  <asp:DataPager ID="DataPager1" runat="server" PagedControlID="ListView1" PageSize="50">
    ...
    <asp:SqlDataSource ID="SqlDataSource3" runat="server" ConnectionString="<%$ ConnectionStrings:DefaultConnection %>" ProviderName="<%$ ConnectionStrings:DefaultConnection.ProviderName %>" SelectCommand="SELECT dbo.Products.ProductKey, dbo.Products.CoverImage, ISNULL(dbo.Products.enabledeal, N'Discount') AS enabledeal, dbo.Products.ProductName, dbo.Products.Price, dbo.Products.Model, dbo.Products.StartDate, dbo.Products.ExpiryDate, dbo.Profile.VendorName, dbo.Profile.VendorLogo, dbo.Profile.ProfileId, dbo.Products.UserApproval, dbo.Products.SubCategoryId, dbo.Products.Brand, dbo.Products.CountryofOrigin, dbo.Products.featurestatus, ISNULL(BusinessViews.Totalview, 0) AS TotalView, ISNULL(favorite.MyProductId, 0) AS favstatus, dbo.Products.discountoffer, dbo.SalesType.SalesType, dbo.Products.actualprice, dbo.Country.Country, dbo.City.CityName, dbo.Condition.Condition, dbo.Condition.ConditionID, ISNULL(dbo.Products.demotime, N'1') AS demotime, ISNULL(dbo.Products.offerexpiry, GETDATE()) AS offerexpiry, dbo.Products.AvailableQuantity, dbo.Currency.Currency, dbo.Category.CategoryName, dbo.SCategory.SCategoryname, nullorder.ProductKey AS Expr1 FROM dbo.SCategory INNER JOIN dbo.Products INNER JOIN dbo.Profile ON dbo.Products.UserName = dbo.Profile.UserName INNER JOIN dbo.SalesType ON dbo.Products.SaleTypeId = dbo.SalesType.SalesTypeID INNER JOIN dbo.Condition ON dbo.Products.ConditionId = dbo.Condition.ConditionID INNER JOIN dbo.Currency ON dbo.Products.Currency = dbo.Currency.CurrencyId INNER JOIN dbo.Category ON dbo.Products.CategoryId = dbo.Category.CategoryId ON dbo.SCategory.ScategoryId = dbo.Products.SubCategoryId LEFT OUTER JOIN dbo.City ON dbo.Profile.city = dbo.City.CityID LEFT OUTER JOIN dbo.Country ON dbo.Profile.country = dbo.Country.Countryid LEFT OUTER JOIN (SELECT ProductKey, Status FROM dbo.[Order] WHERE (Status = N'Active')) AS nullorder ON dbo.Products.ProductKey = nullorder.ProductKey LEFT OUTER JOIN (SELECT MyProductId FROM dbo.Favorite AS Favorite_1 WHERE (UserName = @UserName)) AS favorite ON dbo.Products.ProductKey = favorite.MyProductId LEFT OUTER JOIN (SELECT BusineessId, COUNT(*) AS Totalview FROM dbo.BusinessViews AS BusinessViews_1 GROUP BY BusineessId) AS BusinessViews ON dbo.Products.ProductKey = BusinessViews.BusineessId WHERE (dbo.Products.AdminApproval = N'Active') AND (dbo.Products.UserApproval = N'Active') AND (dbo.Products.SubCategoryId IN (SELECT CAST(Value AS int) AS Expr1 FROM dbo.udf_Split(@CategoryID, ',') AS udf_Split_1)) AND (dbo.Products.Price BETWEEN @min AND @max) AND (ISNULL(dbo.Profile.VendorName, N'') LIKE N'%' + @Vendor + N'%') AND (dbo.Products.ProductName + N' ' + ISNULL(dbo.Products.CountryofOrigin, N'') + ' ' + ISNULL(dbo.Profile.VendorName, N'') LIKE N'%' + @Keyword + N'%') AND (nullorder.ProductKey IS NULL) ORDER BY CASE WHEN @sort = 'PN' THEN Products.ProductName END, CASE WHEN @sort = 'F' THEN Products.featurestatus END DESC, CASE WHEN @sort = 'P' THEN Products.Price END, CASE WHEN @sort = 'V' THEN ISNULL(BusinessViews.Totalview , 0) END DESC" OnSelecting="SqlDataSource3_Selecting">
        <SelectParameters>
            ...
            <asp:Parameter DefaultValue="0" Name="min" />
            <asp:Parameter DefaultValue="100000" Name="max" />
            <asp:Parameter DefaultValue="%" Name="Vendor" />
            <asp:Parameter DefaultValue="PN" Name="sort" />
