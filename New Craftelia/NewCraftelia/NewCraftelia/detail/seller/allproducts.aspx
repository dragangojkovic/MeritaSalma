...
<div class="col-md-3" style="margin-bottom:10px;">
                          ...
                          <div class="form-div">
                                <asp:Label ID="Label9" runat="server" Text='<%# Eval("SalesType") %>'  ></asp:Label> <asp:LinkButton ID="LinkButton2" runat="server" CssClass="text text-info pull-right" CommandName="Auction" Visible="false" Font-Size="11px" Font-Bold="True"></asp:LinkButton>
  ...
  <asp:LinkButton ID="LinkButton4" runat="server" CssClass="text-danger" CommandName="Edit" >Edit</asp:LinkButton>    <asp:LinkButton ID="LinkButton5" runat="server" CommandName="Feature" CssClass="text-success" Visible="False">Featured</asp:LinkButton> <br />
      ...
      <asp:SqlDataSource ID="SqlDataSource3" runat="server" ConnectionString="<%$ ConnectionStrings:DefaultConnection %>" ProviderName="<%$ ConnectionStrings:DefaultConnection.ProviderName %>" SelectCommand="SELECT Products.ProductKey, Products.CoverImage, Products.ProductName, Products.Currency, Products.Price, Products.Model, Products.StartDate, Products.ExpiryDate, Products.AdminApproval, Category.CategoryName, SCategory.SCategoryname, ISNULL(Products.featurestatus, 'No') AS FeatureStatus, Products.UserName, Products.UserApproval, SalesType.SalesType FROM Products INNER JOIN Category ON Products.CategoryId = Category.CategoryId INNER JOIN SCategory ON Products.SubCategoryId = SCategory.ScategoryId INNER JOIN SalesType ON Products.SaleTypeId = SalesType.SalesTypeID WHERE (ISNULL(Products.featurestatus, N'No') LIKE '%' + @FeatureStatus + '%') AND (Products.AdminApproval LIKE '%' + @AdminApproval + '%') AND (Category.CategoryName LIKE N'%' + @Category + N'%') AND (SCategory.SCategoryname LIKE N'%' + @SubCategory + N'%') AND (Products.UserName = @UserName) AND (SalesType.SalesType LIKE N'%' + @Salestype + N'%') ORDER BY Products.ProductName" UpdateCommand="UPDATE dbo.Products SET featurestatus = @FeatureStatus WHERE (ProductKey = @ID)">
          <SelectParameters>
