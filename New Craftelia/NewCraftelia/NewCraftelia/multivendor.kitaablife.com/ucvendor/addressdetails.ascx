    ...
    <uc1:savediv runat="server" ID="savediv" Visible="false" />
                     ...
                     <div class="form-group"> 
                           ...
                           <asp:Label ID="Label8" runat="server" Text="Country"></asp:Label> 
                                   <asp:DropDownList ID="DropDownList1" runat="server" AppendDataBoundItems="True" DataSourceID="SqlDataSource6" DataTextField="Country" DataValueField="CountryId" AutoPostBack="True" >
                ...
                if (status == google.maps.GeocoderStatus.OK) {
                    ...
                    google.maps.event.addListener(marker, "click", function (e) {
                        ...
                        infoWindow.open(map, marker);
                         ...
                         <br />
<asp:SqlDataSource ID="SqlDataSource5" runat="server" ConnectionString="<%$ ConnectionStrings:DefaultConnection %>" ProviderName="<%$ ConnectionStrings:DefaultConnection.ProviderName %>" SelectCommand="SELECT ISNULL(country, '0') AS Country, ISNULL(cityname, '') AS City, ISNULL(District, '') AS District, ISNULL(Street, '') AS Street, ISNULL(Building, '') AS Building, ISNULL(googlemap, '') AS googlemap, ISNULL(postcode, '') AS postcode FROM dbo.Profile WHERE (UserName = @UserName)" UpdateCommand="UPDATE dbo.Profile SET country = @Country, District = @District, Street = @Street, Building = @Building, googlemap = @googlemap, postcode = @postcode, cityname = @CityName WHERE (UserName = @UserName)">
    <SelectParameters>
