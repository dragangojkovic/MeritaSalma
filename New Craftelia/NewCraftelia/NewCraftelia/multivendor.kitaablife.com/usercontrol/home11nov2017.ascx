...
<span class="pull-right" style="margin-top:-15px">
    ...
    <asp:SqlDataSource ID="SqlDataSource3" runat="server" ConnectionString="<%$ ConnectionStrings:DefaultConnection %>" ProviderName="<%$ ConnectionStrings:DefaultConnection.ProviderName %>" SelectCommand="SELECT TOP (12) dbo.Products.ProductKey, dbo.Products.CoverImage, ISNULL(dbo.Products.enabledeal, N'Discount') AS enabledeal, dbo.Products.ProductName, dbo.Products.Price, dbo.Products.Model, dbo.Products.StartDate, dbo.Products.ExpiryDate, dbo.Profile.VendorName, dbo.Profile.VendorLogo, dbo.Profile.ProfileId, dbo.Products.UserApproval, dbo.Products.SubCategoryId, dbo.Products.Brand, dbo.Products.CountryofOrigin, dbo.Products.featurestatus, ISNULL(BusinessViews.Totalview, 0) AS TotalView, ISNULL(favorite.MyProductId, 0) AS favstatus, dbo.Products.discountoffer, dbo.SalesType.SalesType, dbo.Products.actualprice, dbo.Country.Country, dbo.City.CityName, dbo.Condition.Condition, dbo.Condition.ConditionID, ISNULL(dbo.Products.demotime, N'1') AS demotime, ISNULL(dbo.Products.offerexpiry, GETDATE()) AS offerexpiry, dbo.Products.AvailableQuantity, dbo.Currency.Currency, dbo.Products.ProductID, dbo.Category.CategoryName, dbo.SCategory.SCategoryname FROM dbo.Currency INNER JOIN dbo.Products INNER JOIN dbo.Profile ON dbo.Products.UserName = dbo.Profile.UserName INNER JOIN dbo.SalesType ON dbo.Products.SaleTypeId = dbo.SalesType.SalesTypeID INNER JOIN dbo.Country ON dbo.Profile.country = dbo.Country.Countryid INNER JOIN dbo.City ON dbo.Profile.city = dbo.City.CityID INNER JOIN dbo.Condition ON dbo.Products.ConditionId = dbo.Condition.ConditionID ON dbo.Currency.CurrencyId = dbo.Products.Currency INNER JOIN dbo.Category ON dbo.Products.CategoryId = dbo.Category.CategoryId INNER JOIN dbo.SCategory ON dbo.Products.SubCategoryId = dbo.SCategory.ScategoryId LEFT OUTER JOIN (SELECT MyProductId FROM dbo.Favorite AS Favorite_1 WHERE (UserName = @UserName)) AS favorite ON dbo.Products.ProductKey = favorite.MyProductId LEFT OUTER JOIN (SELECT BusineessId, COUNT(*) AS Totalview FROM dbo.BusinessViews AS BusinessViews_1 GROUP BY BusineessId) AS BusinessViews ON dbo.Products.ProductKey = BusinessViews.BusineessId WHERE (dbo.Products.AdminApproval = N'Active') AND (dbo.Products.UserApproval = N'Active') ORDER BY dbo.Products.ProductID DESC">
        <SelectParameters>
