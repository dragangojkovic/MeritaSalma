...
<div class="col-md-6">
    ...
    <div class="row">
        ...
        <div class="col-md-3 nopadding form" style="text-align: right">
            <asp:TextBox ID="TextBox1" runat="server" Text="1" Visible="False"></asp:TextBox>
...
<asp:RequiredFieldValidator ID="RequiredFieldValidator1" runat="server" ErrorMessage="Quantity Must Be Greater than 0" ValidationGroup="book" InitialValue="0" ControlToValidate="TextBox1" CssClass="required" Display="Static"></asp:RequiredFieldValidator>
    ...
    <asp:SqlDataSource ID="SqlDataSource23" runat="server" ConnectionString="<%$ ConnectionStrings:DefaultConnection %>" InsertCommand="INSERT INTO dbo.[Order] SELECT DISTINCT @OrderNo AS OrderNo, (SELECT Id FROM dbo.AspNetUsers WHERE (UserName = @buyerusername)) AS buyerid, (SELECT dbo.AspNetUsers.Id FROM dbo.Profile INNER JOIN dbo.AspNetUsers ON dbo.Profile.UserName = dbo.AspNetUsers.UserName WHERE (dbo.Profile.ProfileId = @sellerusername)) AS sellerid, @ID AS ProductKey, GETDATE() AS OrderDate, (SELECT Price FROM dbo.Products WHERE (ProductKey = @ID)) AS Amount, @Status AS Status, @Requirement AS Requirement, GETDATE() AS ActionDate, 0 AS DeliveryId, 'NA' AS Comments, @Quantity AS Quantity, @paymenttype AS paymenttype, @commissionper AS commissionpercentage, 0 AS commissionamount, @deliverycostperitem as deliverycostperitem  FROM dbo.AspNetUsers" ProviderName="<%$ ConnectionStrings:DefaultConnection.ProviderName %>" SelectCommand="SELECT ISNULL(availablebalance.availablefunds - serviceamount.MinAmount, 0) AS balanceforshopping FROM (SELECT MinAmount FROM CreateService WHERE (CreateServiceId = @ID)) AS serviceamount CROSS JOIN (SELECT balancefund.funds - orderfund.orderfund AS availablefunds FROM (SELECT ISNULL(SUM(Funds), 0) AS funds FROM AddBalance WHERE (BuyerId = (SELECT Id FROM dbo.AspNetUsers WHERE (UserName = @username)))) AS balancefund CROSS JOIN (SELECT ISNULL(SUM(Amount), 0) AS orderfund FROM [Order] WHERE (Status &lt;&gt; N'Pending') AND (Status &lt;&gt; 'Cancel') AND (BuyerId = (SELECT Id FROM dbo.AspNetUsers AS AspNetUsers_1 WHERE (UserName = @username)))) AS orderfund) AS availablebalance" UpdateCommand="UPDATE dbo.[Order] SET commissionamount = Amount / commissionper - Amount WHERE (ProductKey = @Key ) AND (OrderNo = @OrderNo)">
    <InsertParameters>
        ...
        <asp:Parameter DefaultValue="NA" Name="paymenttype" />
        <asp:SessionParameter Name="commissionper" SessionField="comm" />
        <asp:SessionParameter DefaultValue="0" Name="deliverycostperitem" SessionField="Deliverycost" />
    </InsertParameters>
    <SelectParameters>
        ...
        <UpdateParameters>
            <asp:RouteParameter Name="Key" RouteKey="ID" />
            <asp:SessionParameter DefaultValue="" Name="OrderNo" SessionField="cart" />
        </UpdateParameters>
</asp:SqlDataSource>
