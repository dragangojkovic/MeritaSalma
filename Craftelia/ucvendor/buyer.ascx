...
<br />
    Your Paypal ID to receive payment from admin:    <asp:TextBox ID="TextBox2" runat="server"></asp:TextBox>
  ...
  <asp:Button ID="Button1" runat="server" ValidationGroup="vendor" Text="Save" OnClick="Button1_Click" OnClientClick="if (!Page_ClientValidate('vendor')){ return false; } this.disabled = true; this.value = 'Processing...';" UseSubmitBehavior="False" />   
    ...
    <asp:PostBackTrigger ControlID="btnUpload" />
                       ...
                       <asp:SqlDataSource ID="SqlDataSource5" runat="server" ConnectionString="<%$ ConnectionStrings:DefaultConnection %>" ProviderName="<%$ ConnectionStrings:DefaultConnection.ProviderName %>" SelectCommand="SELECT ISNULL(VendorName, '') AS VendorName, ISNULL(VendorLogo, '') AS VendorLogo, isnull(paypal,'') as paypal FROM dbo.Profile WHERE (UserName = @UserName)" UpdateCommand="UPDATE dbo.Profile SET VendorName = @VendorName, VendorLogo = @VendorLogo, paypal = @paypal WHERE (UserName = @UserName)">
                           <SelectParameters>
                           ...
                           <UpdateParameters>
                               ...
                               <asp:ControlParameter ControlID="TextBox2" Name="paypal" PropertyName="Text" />
                           </UpdateParameters>
                       </asp:SqlDataSource>
