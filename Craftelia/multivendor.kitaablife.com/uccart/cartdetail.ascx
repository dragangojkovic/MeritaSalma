...
<asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="<%$ ConnectionStrings:DefaultConnection %>" ProviderName="<%$ ConnectionStrings:DefaultConnection.ProviderName %>" SelectCommand="SELECT dbo.[Order].OrderNo, dbo.Products.CoverImage, dbo.Products.ProductName, dbo.[Order].Quantity, dbo.[Order].Amount, dbo.[Order].Comments, dbo.[Order].Quantity * dbo.[Order].Amount AS TotalPrice, dbo.[Order].OrderId, dbo.Products.wholesaleqty, dbo.Products.wholesaleprice, dbo.SalesType.SalesType, dbo.Products.Price, dbo.Currency.Currency FROM dbo.[Order] INNER JOIN dbo.Products ON dbo.[Order].ProductKey = dbo.Products.ProductKey INNER JOIN dbo.SalesType ON dbo.Products.SaleTypeId = dbo.SalesType.SalesTypeID INNER JOIN dbo.Currency ON dbo.Products.Currency = dbo.Currency.CurrencyId WHERE (dbo.[Order].OrderNo = @ID)" DeleteCommand="DELETE FROM dbo.[Order] WHERE (OrderId = @ID)" UpdateCommand="UPDATE [Order] SET Quantity = @Quantity, Amount = @Amount WHERE (OrderId = @ID)">
        <SelectParameters>
